...
func TestGenerateDedicatedPolicyNameIsStable(t *testing.T) {
	...
	if err := derivePolicyNames(p); err != nil { t.Fatal(err) }
	rule := &p.Services[0].Rules[0]
	first := rule.PolicyName
	if strings.Contains(first, "M120") { t.Fatalf("dedicated name contains MKZ: %q", first) }
	if len(first) > 35 || !strings.HasPrefix(first, "SRV_GDMZ_IDMZ_sql_in_") { t.Fatalf("unexpected name %q", first) }
	rule.Owner, rule.ChangeReference = "NEW", "CHG-2"
	if err := derivePolicyNames(p); err != nil { t.Fatal(err) }
	if rule.PolicyName != first { t.Fatalf("name changed from %q to %q", first, rule.PolicyName) }
}
...
func TestGenerateSharedPolicyNameAndIgnoreClientName(t *testing.T) {
	...
	if !strings.HasPrefix(got, "SRV_M120_GDMZ_IDMZ_sql_in_") { t.Fatalf("unexpected shared name %q", got) }
	if got == "CLIENT_CONTROLLED" { t.Fatal("client policy name was accepted") }
}

func TestPolicyNamingValidation(t *testing.T) {
	p := namingTestPolicy("shared")
	p.Tenants[0].MKZ = "M000"
	if err := derivePolicyNames(p); err == nil { t.Fatal("M000 was accepted") }
	p = namingTestPolicy("shared")
	p.Networks[0].Zone = ""
	if err := derivePolicyNames(p); err == nil { t.Fatal("missing zone was accepted") }
}
...
func TestShortIDCollisionIsResolvedDeterministically(t *testing.T) {
	...
	if a.ShortID == b.ShortID { t.Fatalf("collision was not resolved: %s", a.ShortID) }
	if err := derivePolicyNames(p); err != nil { t.Fatal(err) }
	if p.Services[0].Rules[1].ShortID != b.ShortID { t.Fatal("resolved ID was not stable") }
}
